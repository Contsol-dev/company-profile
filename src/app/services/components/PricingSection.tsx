...
const PricingSection = ({ packages }: PricingSectionProps) => {
  return (
    <section className="flex flex-col items-center p-10 bg-gray-100">
      <div className={`grid grid-cols-1 md:grid-cols-${packages.length} gap-8`}>
        {packages.map((pkg, index) => (
          <div
            key={index}
            className={`${
              index + 1 === packages.length ? "bg-sky-900" : "bg-white"
          ...
          >
            <h3
              className={`${
                index + 1 === packages.length ? "text-white" : "text-sky-900"
            ...
            <ul
              className={`${
                index + 1 === packages.length ? "text-white" : "text-gray-900"
